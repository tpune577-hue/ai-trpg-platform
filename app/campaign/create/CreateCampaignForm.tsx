...
export default function CreateCampaignForm({ isApprovedSeller }: Props) {
    ...
    const handleSave = async (isDraft: boolean) => {
        ...
        // 🛡️ Seller Validation
        if (!isApprovedSeller && price > 0) {
            return alert("⛔ You must be an Approved Seller to set a price greater than 0.")
        }
                                ...
                                <InputGroup label="Price (Coins)">
                                    ...
                                    {isPriceError && (
                                        <div className="mt-3 p-3 bg-red-900/20 border border-red-900/50 rounded-lg text-sm text-red-300">
                                            ...
                                            <div className="mt-1">
                                                <Link href="/seller/register" className="underline hover:text-white font-bold">Register as Seller</Link> or set price to 0.
                                            </div>
                                        </div>
                                    )}
                                </InputGroup>
